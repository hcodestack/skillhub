...
const SEV_KEY: Record<string, MsgKey> = {
  error: 'he.sev.error', warn: 'he.sev.warn', info: 'he.sev.info',
};

const JOB_STATUS: Record<string, { label: MsgKey; cls: string }> = {
  ok: { label: 'he.job.ok', cls: 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300' },
  failed: { label: 'he.job.failed', cls: 'bg-red-500/15 text-red-700 dark:text-red-300' },
  stuck: { label: 'he.job.stuck', cls: 'bg-red-500/15 text-red-700 dark:text-red-300' },
  running: { label: 'he.job.running', cls: 'bg-sky-500/15 text-sky-700 dark:text-sky-300' },
  never: { label: 'he.job.never', cls: 'bg-foreground/10 text-foreground/60' },
};

function fmtDur(s: number): string {
  if (s < 1) return t('he.dur.subSecond');
  if (s < 60) return t('he.dur.seconds', { n: s });
  return t('he.dur.minutes', { m: Math.floor(s / 60), s: s % 60 });
}
...
function Items({ section }: { section: HealthSection }) {
  ...
  if (!items.length) {
    return <p className="text-sm text-foreground/60">{t('common.none')}</p>;
  }
      ...
      <div className="flex flex-wrap gap-1">
        ...
        {items.length > shown.length && (
          <span className="px-1.5 py-0.5 text-xs text-foreground/60">
            {t('he.item.more', { n: items.length - shown.length })}
          </span>
        )}
      </div>
    );
          ...
          <li key={i} className="flex flex-wrap items-baseline gap-2 text-sm">
            ...
            {it.chars != null && (
              <span className="tabular-nums text-foreground/60">
                {t('he.item.chars', { n: String(it.chars) })}
              </span>
            )}
            {it.lines != null && (
              <span className="tabular-nums text-foreground/60">
      ...
      <ul className="flex flex-col gap-1.5">
        {items.map((it, i) => (
          <li key={i} className="flex flex-wrap items-baseline gap-2 text-sm">
            ...
            <span className="tabular-nums text-foreground/60">
              {t('he.item.copies', { n: String(it.copies) })}
            </span>
            {it.drifted ? (
              <Chip size="sm" className="bg-amber-500/15 text-amber-700 dark:text-amber-300">
                {t('he.item.drifted', { n: String(it.variants) })}
              </Chip>
            ) : (
      ...
      <ul className="flex flex-col gap-1.5">
        {items.map((it, i) => (
          <li key={i} className="flex flex-wrap items-baseline gap-2 text-sm">
            ...
            <span className="tabular-nums text-foreground/60">
              {t('he.item.times', { n: String(it.n) })}
            </span>
            <span className="text-xs text-foreground/60">{String(it.agents ?? '')}</span>
            <span className="text-xs text-foreground/60">
              {t('he.item.last', { when: fmtRel(it.last as string) })}
            </span>
          </li>
        ))}
      </ul>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {it.scope != null && (
              <Chip size="sm" className="bg-foreground/10">
                {t(it.scope === 'global' ? 'common.global' : 'common.project')}
              </Chip>
            )}
            <span className="font-medium">{String(it.entry_name ?? '')}</span>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-xs text-foreground/60">{t('he.intro')}</span>
        <LinkDocToggle />
        ...
        <span className="ml-auto flex items-center gap-2">
          <a className="rounded-md border border-foreground/15 px-2 py-0.5 text-xs text-foreground/70 transition-colors hover:border-foreground/30 hover:text-foreground"
             href={`/api/v1/report/governance.md?lang=${lang}`}
             download={t('he.export.reportFile')}>
            {t('he.export.report')}
          </a>
          <a className="rounded-md border border-foreground/15 px-2 py-0.5 text-xs text-foreground/70 transition-colors hover:border-foreground/30 hover:text-foreground"
             href={`/api/v1/report/remediation.sh?lang=${lang}`}
             download={t('he.export.scriptFile')}
             title={t('he.export.scriptTitle')}>
            {t('he.export.script')}
          </a>
        </span>
      </div>
            ...
            <Card.Header className={expanded ? 'pb-2' : 'pb-4'}>
              ...
              >
                <Chip size="sm" className={SEV_CLASS[s.severity] ?? SEV_CLASS.info}>
                  {SEV_KEY[s.severity] ? t(SEV_KEY[s.severity]) : s.severity}
                </Chip>
